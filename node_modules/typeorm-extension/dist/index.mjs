...
import { load, isObject, removeFileNameExtension, locate, getModuleExport, locateMany, getFileNameExtension } from 'locter';
...
async function findDataSource(context) {
    ...
    if (!context.preserveFilePaths && !isCodeTransformation(CodeTransformation.JUST_IN_TIME)) {
        for(let j = 0; j < files.length; j++){
    ...
    for(let i = 0; i < files.length; i++){
        ...
        if (info) {
            ...
            if (defaultExport && InstanceChecker.isDataSource(defaultExport.value)) {
...
class SeederEntity {
    trackExecution() {
        return !!this.instance && typeof this.instance.track === 'boolean' && this.instance.track;
    }
    constructor(ctx){
        this.id = ctx.id;
